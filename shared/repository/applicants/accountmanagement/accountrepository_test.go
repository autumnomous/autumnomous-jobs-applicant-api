...
func Test_ApplicantRepository_GetApplicant(t *testing.T) {
	assert := assert.New(t)
	...
	result, err := repository.GetApplicant(applicant.PublicID)

	assert.Equal(result.FirstName, applicant.FirstName)
	assert.Equal(result.LastName, applicant.LastName)
	assert.Equal(result.Email, applicant.Email)
	assert.NotNil(result.PublicID)
	assert.Nil(err)

}

func Test_ApplicantRepository_GetApplicant_Fail_EmptyData(t *testing.T) {
	assert := assert.New(t)

	repository := accountmanagement.NewApplicantRepository(database.DB)

	result, err := repository.GetApplicant("")
